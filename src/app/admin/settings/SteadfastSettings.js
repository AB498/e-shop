...
export default function SteadfastSettings() {
  ...
  const [autoCreateSteadfastOrder, setAutoCreateSteadfastOrder] = useState(false);
  ...
  useEffect(() => {
    async function fetchSettings() {
      try {
        ...
        // Find auto_create_steadfast_order setting
        const autoCreateSetting = data.find(setting => setting.key === 'auto_create_steadfast_order');
        if (autoCreateSetting) {
          setAutoCreateSteadfastOrder(autoCreateSetting.value === 'true');
        }
      } catch (error) {
  ...
  const handleToggleChange = async () => {
    try {
      setSaving(true);
      const newValue = !autoCreateSteadfastOrder;
      ...
      setAutoCreateSteadfastOrder(newValue);
    } catch (error) {
    ...
    <div className="bg-white shadow overflow-hidden rounded-lg">
      <div className="px-3 py-3 md:px-4 md:py-5 sm:px-6 bg-gray-50">
        ...
        <p className="mt-1 max-w-2xl text-xs md:text-sm text-gray-500">
          Configure automatic courier order creation with Steadfast
        </p>
      </div>
          ...
          <div className="flex flex-row items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs md:text-sm text-gray-500">
                Automatically create Steadfast courier orders when a new order is placed
              </p>
            </div>
            <div className="ml-4 flex-shrink-0">
              <button
                ...
                disabled={saving}
                className={`${
                  autoCreateSteadfastOrder ? 'bg-emerald-600' : 'bg-gray-200'
                ...
                role="switch"
                aria-checked={autoCreateSteadfastOrder}
              >
                <span
                  aria-hidden="true"
                  className={`${
                    autoCreateSteadfastOrder ? 'translate-x-5' : 'translate-x-0'
