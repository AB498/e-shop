...
async function handlePaymentFail(request, isPost = false) {
  try {
    // Log the full request URL for debugging
    console.log(`Payment fail callback URL (${isPost ? 'POST' : 'GET'}):`, request.url);
    ...
    if (isPost) {
      ...
      try {
        ...
        console.log('Payment fail POST data:', Object.fromEntries(formData.entries()));
        ...
        try {
          ...
          console.log('Payment fail POST JSON data:', jsonData);
    ...
    if (!orderId) {
      ...
      console.log('Payment fail parameters from URL:', Object.fromEntries(searchParams.entries()));
    ...
    if (!orderId) {
      console.error('Missing order_id in payment fail callback');
      try {
        ...
        return NextResponse.redirect(`http://localhost:3000/payment/error?message=Invalid order reference`, { status: 303 });
      }
    }
    ...
    try {
      ...
      // Continue anyway, as we still want to show the failure page
    }

    // Redirect to failure page
    try {
      const redirectUrl = `${baseUrl}/payment/failed?order_id=${orderId}`;
      ...
      // Fallback to a hardcoded URL if there's an issue
      return NextResponse.redirect(`http://localhost:3000/payment/failed?order_id=${orderId}`, { status: 303 });
    }
...
export async function GET(request) {
  return handlePaymentFail(request, false);
}

// POST handler
export async function POST(request) {
  return handlePaymentFail(request, true);
}
