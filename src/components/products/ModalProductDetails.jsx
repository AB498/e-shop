...
const ModalProductDetails = ({
  ...
  return (
    <div className="w-full md:w-3/5 p-2 sm:p-1.5 md:p-2">
      {/* Product title */}
      <h2 className="text-base sm:text-base md:text-lg lg:text-xl font-semibold text-black mb-2 sm:mb-1.5 md:mb-2 leading-snug">
      ...
      {/* Price */}
      <div className="mb-2 sm:mb-1.5 md:mb-2">
        <p className="text-[#006B51] text-sm md:text-sm font-bold">
          {product.discountPrice !== product.price ? (
            <>
              <span className="text-[#E12625] text-xs sm:text-xs line-through mr-1 sm:mr-1">
      ...
      {/* Available sizes */}
      <div className="mb-2 sm:mb-1.5 md:mb-2 border-b border-gray-200 pb-2 sm:pb-1.5 md:pb-2">
        <p className="text-[rgba(0,0,0,0.7)] text-xs sm:text-[10px] mb-1 sm:mb-1 uppercase">available in:</p>
        <div className="flex flex-wrap gap-1 sm:gap-1">
          {['small', 'medium', 'large'].map((size) => (
            <button
              key={size}
              onClick={() => changeSize(size)}
              className={`px-2 py-0.5 sm:px-1.5 sm:py-0.5 md:px-2 md:py-0.5 border rounded-full text-xs sm:text-[10px] font-semibold ${
                selectedSize === size
      ...
      {/* Add to cart section */}
      <div className="flex flex-col gap-1 sm:gap-1 md:gap-1.5">
        <div className="flex items-center gap-1 sm:gap-1 flex-wrap">
          {/* Quantity selector */}
          <div className="flex items-center bg-[#F3F5F9] rounded-full h-7 sm:h-6 md:h-7">
            <button
              onClick={decreaseQuantity}
              className="p-1 rounded-full"
              aria-label="Decrease quantity"
            >
              <svg width="12" height="12" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-2.5 h-2.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
                <path d="M4 10H16" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
            <span className="px-2 sm:px-1.5 md:px-2 font-semibold text-xs sm:text-[10px]">{quantity}</span>
            <button
          ...
          <button
            ...
            className="flex items-center justify-center gap-1 bg-[#006B51] text-white flex-1 rounded-full font-semibold hover:bg-[#005541] transition-colors tracking-widest h-7 sm:h-6 md:h-7 px-2 sm:px-1.5 md:px-2"
          >
            <svg width="12" height="12" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-2.5 h-2.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
            ...
            <span className="text-xs sm:text-[10px]">Add to Cart</span>
          </button>
        </div>

        {/* Wishlist and Share buttons */}
        <div className="flex flex-wrap gap-1 sm:gap-1 mt-1 sm:mt-1">
          <button
            onClick={() => {
            ...
            className={`flex items-center justify-center gap-1 border border-[#B1AEA9] rounded-full flex-1 h-7 sm:h-6 md:h-7 px-2 sm:px-1.5 md:px-2 ${
              productInWishlist ? 'bg-[#F0F7F5]' : 'bg-white'
            }`}
            aria-label={productInWishlist ? "Remove from wishlist" : "Add to wishlist"}
          >
            {isWishlistLoading ? (
              <div className="w-2.5 h-2.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5 border-[1.5px] border-[#FF3E3E] border-t-transparent rounded-full animate-spin"></div>
            ) : (
              <svg width="12" height="12" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-2.5 h-2.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
            ...
            <span className="text-xs sm:text-[10px] font-semibold">Wishlist</span>
          </button>
          <button
            className="flex items-center justify-center gap-1 border border-[#B1AEA9] rounded-full flex-1 bg-white h-7 sm:h-6 md:h-7 px-2 sm:px-1.5 md:px-2"
            aria-label="Share product"
          >
            <svg width="12" height="12" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-2.5 h-2.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
      ...
      {/* Tags */}
      <div className="mt-2 sm:mt-1.5 md:mt-2 flex flex-col">
        <div className="flex items-center gap-1 mb-1 sm:mb-1">
          <svg width="12" height="12" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-2.5 h-2.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
          ...
          <span className="text-[rgba(0,0,0,0.8)] text-xs sm:text-[10px]">Tags:</span>
        </div>
        <div className="flex flex-wrap gap-1">
          {['Beauty & Care', 'Hair Treatment', 'Tag Name', 'Tag Name', 'Tag Name'].map((tag, index) => (
            <span
              key={index}
              className="px-1.5 py-0.5 sm:px-1 sm:py-0.5 border border-[#B1AEA9] rounded-full text-xs sm:text-[9px] text-[#595959] font-medium"
            >
              {tag}
            </span>
          ))}
      ...
      {/* Product details */}
      <div className="mt-2 sm:mt-1.5 md:mt-2">
        <h3 className="font-semibold text-black mb-1 text-xs sm:text-[10px]">Product Details:</h3>
        <p className="text-[#595959] text-xs sm:text-[9px] leading-relaxed">
