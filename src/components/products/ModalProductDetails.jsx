...
const ModalProductDetails = ({
  ...
  return (
    <div className="w-full md:w-3/5 p-1 sm:p-1.5 md:p-2">
      {/* Product title */}
      <h2 className="text-sm sm:text-base md:text-lg lg:text-xl font-semibold text-black mb-1 sm:mb-1.5 md:mb-2 leading-snug">
      ...
      <div className="mb-1 sm:mb-1.5 md:mb-2">
        <p className="text-[#006B51] text-xs md:text-sm font-bold">
          {product.discountPrice !== product.price ? (
            <>
              <span className="text-[#E12625] text-[10px] sm:text-xs line-through mr-0.5 sm:mr-1">
      ...
      {/* Available sizes */}
      <div className="mb-1 sm:mb-1.5 md:mb-2 border-b border-gray-200 pb-1 sm:pb-1.5 md:pb-2">
        <p className="text-[rgba(0,0,0,0.7)] text-[9px] sm:text-[10px] mb-0.5 sm:mb-1 uppercase">available in:</p>
        <div className="flex flex-wrap gap-0.5 sm:gap-1">
          {['small', 'medium', 'large'].map((size) => (
            <button
              key={size}
              onClick={() => changeSize(size)}
              className={`px-1 py-0 sm:px-1.5 sm:py-0.5 md:px-2 md:py-0.5 border rounded-full text-[9px] sm:text-[10px] font-semibold ${
                selectedSize === size
      ...
      <div className="flex flex-col gap-0.5 sm:gap-1 md:gap-1.5">
        <div className="flex items-center gap-0.5 sm:gap-1 flex-wrap">
          {/* Quantity selector */}
          <div className="flex items-center bg-[#F3F5F9] rounded-full h-5 sm:h-6 md:h-7">
            <button
              onClick={decreaseQuantity}
              className="p-0.5 rounded-full"
              aria-label="Decrease quantity"
            >
              <svg width="10" height="10" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-1.5 h-1.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
                <path d="M4 10H16" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
            <span className="px-1 sm:px-1.5 md:px-2 font-semibold text-[9px] sm:text-[10px]">{quantity}</span>
            <button
          ...
          <button
            ...
            className="flex items-center justify-center gap-0.5 bg-[#006B51] text-white flex-1 rounded-full font-semibold hover:bg-[#005541] transition-colors tracking-widest h-5 sm:h-6 md:h-7 px-1 sm:px-1.5 md:px-2"
          >
            <svg width="10" height="10" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-1.5 h-1.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
            ...
            <span className="text-[9px] sm:text-[10px]">Add to Cart</span>
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-0.5 sm:gap-1 mt-0.5 sm:mt-1">
          <button
            ...
            className={`flex items-center justify-center gap-0.5 border border-[#B1AEA9] rounded-full flex-1 h-5 sm:h-6 md:h-7 px-1 sm:px-1.5 md:px-2 ${
              productInWishlist ? 'bg-[#F0F7F5]' : 'bg-white'
            }`}
            aria-label={productInWishlist ? "Remove from wishlist" : "Add to wishlist"}
          >
            {isWishlistLoading ? (
              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5 border-[1.5px] border-[#FF3E3E] border-t-transparent rounded-full animate-spin"></div>
            ) : (
              <svg width="10" height="10" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-1.5 h-1.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
          ...
          <button
            className="flex items-center justify-center gap-0.5 border border-[#B1AEA9] rounded-full flex-1 bg-white h-5 sm:h-6 md:h-7 px-1 sm:px-1.5 md:px-2"
            aria-label="Share product"
          >
            <svg width="10" height="10" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-1.5 h-1.5 sm:w-2 sm:h-2 md:w-2.5 md:h-2.5">
            ...
            <span className="text-[9px] sm:text-[10px] font-semibold">Share</span>
          </button>
        </div>
      </div>

      {/* Tags */}
      <div className="mt-1 sm:mt-1.5 md:mt-2 flex flex-col">
        <div className="flex items-center gap-0.5 mb-0.5 sm:mb-1">
          ...
          <span className="text-[rgba(0,0,0,0.8)] text-[9px] sm:text-[10px]">Tags:</span>
        </div>
        <div className="flex flex-wrap gap-0.5">
          {['Beauty & Care', 'Hair Treatment', 'Tag Name', 'Tag Name', 'Tag Name'].map((tag, index) => (
            <span
              key={index}
              className="px-0.5 py-0 sm:px-1 sm:py-0.5 border border-[#B1AEA9] rounded-full text-[8px] sm:text-[9px] text-[#595959] font-medium"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="mt-1 sm:mt-1.5 md:mt-2">
        <h3 className="font-semibold text-black mb-0.5 text-[9px] sm:text-[10px]">Product Details:</h3>
        <p className="text-[#595959] text-[8px] sm:text-[9px] leading-relaxed">
          ...
          <span className="text-[#006B51] font-medium cursor-pointer ml-0.5">Read More</span>
        </p>
      </div>
    </div>
  );
