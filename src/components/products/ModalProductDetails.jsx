...
const ModalProductDetails = ({
  ...
  return (
    <div className="w-full md:w-3/5 p-2 sm:p-3 md:p-5">
      {/* Product title */}
      <h2 className="text-responsive-lg sm:text-responsive-xl md:text-responsive-2xl font-semibold text-black mb-2 sm:mb-3 md:mb-5 leading-snug">
      ...
      <div className="mb-3 sm:mb-4 md:mb-6">
        <p className="text-[#006B51] text-responsive md:text-responsive-lg lg:text-responsive-xl font-bold">
          {product.discountPrice !== product.price ? (
            <>
              <span className="text-[#E12625] text-responsive-xs sm:text-responsive md:text-responsive-lg line-through mr-1 sm:mr-2">
      ...
      {/* Available sizes */}
      <div className="mb-3 sm:mb-4 md:mb-6 border-b border-gray-200 pb-3 sm:pb-4 md:pb-6">
        <p className="text-[rgba(0,0,0,0.7)] text-responsive-xs mb-2 sm:mb-3 uppercase">available in:</p>
        <div className="flex flex-wrap gap-2 sm:gap-3">
          {['small', 'medium', 'large'].map((size) => (
            <button
              key={size}
              onClick={() => changeSize(size)}
              className={`px-2 py-1 sm:px-3 sm:py-1.5 md:px-4 md:py-2 border rounded-full text-responsive-xs font-semibold ${
                selectedSize === size
      ...
      <div className="flex flex-col gap-2 sm:gap-3 md:gap-4">
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          {/* Quantity selector */}
          <div className="flex items-center bg-[#F3F5F9] rounded-full h-8 sm:h-9 md:h-10">
            <button
            ...
            <span className="px-2 sm:px-3 md:px-4 lg:px-6 font-semibold text-responsive-xs sm:text-responsive">{quantity}</span>
            <button
          ...
          <button
            ...
            className="flex items-center justify-center gap-1 sm:gap-2 bg-[#006B51] text-white flex-1 rounded-full font-semibold hover:bg-[#005541] transition-colors tracking-widest h-8 sm:h-9 md:h-10 px-3 sm:px-4 md:px-6"
          >
            <svg width="16" height="16" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 mt-2 sm:mt-3">
          <button
            ...
            className={`flex items-center justify-center gap-1 sm:gap-2 border border-[#B1AEA9] rounded-full flex-1 h-8 sm:h-9 md:h-10 px-3 sm:px-4 md:px-6 ${
              productInWishlist ? 'bg-[#F0F7F5]' : 'bg-white'
            }`}
            aria-label={productInWishlist ? "Remove from wishlist" : "Add to wishlist"}
          >
            <svg width="16" height="16" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5">
          ...
          <button
            className="flex items-center justify-center gap-1 sm:gap-2 border border-[#B1AEA9] rounded-full flex-1 bg-white h-8 sm:h-9 md:h-10 px-3 sm:px-4 md:px-6"
      ...
      {/* Tags */}
      <div className="mt-4 sm:mt-6 md:mt-8 flex flex-col">
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2">
          {['Beauty & Care', 'Hair Treatment', 'Tag Name', 'Tag Name', 'Tag Name'].map((tag, index) => (
            <span
              key={index}
              className="px-2 py-0.5 sm:px-3 sm:py-1 border border-[#B1AEA9] rounded-full text-responsive-xs text-[#595959] font-medium text-xs sm:text-sm"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="mt-4 sm:mt-6 md:mt-8">
        <h3 className="font-semibold text-black mb-2 sm:mb-3 text-responsive-xs sm:text-responsive">Product Details:</h3>
        <p className="text-[#595959] text-responsive-xs leading-relaxed text-xs sm:text-sm">
