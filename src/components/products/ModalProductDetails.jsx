...
const ModalProductDetails = ({
  ...
  return (
    <div className="w-full md:w-3/5 fluid-p">
      {/* Product title */}
      <h2 className="text-responsive-xl md:text-responsive-2xl font-semibold text-black mb-4 md:mb-5 leading-snug">
      ...
      {/* Price */}
      <div className="mb-6">
        <p className="text-[#006B51] text-responsive-lg md:text-responsive-xl font-bold">
          {product.discountPrice !== product.price ? (
            <>
              <span className="text-[#E12625] text-responsive md:text-responsive-lg line-through mr-2">
      ...
      <div className="mb-6 border-b border-gray-200 pb-6">
        <p className="text-[rgba(0,0,0,0.7)] text-responsive-xs mb-3 uppercase">available in:</p>
        <div className="flex flex-wrap fluid-gap">
          {['small', 'medium', 'large'].map((size) => (
            <button
              key={size}
              onClick={() => changeSize(size)}
              className={`px-4 py-2 border rounded-full text-responsive-xs font-semibold ${
                selectedSize === size
      ...
      <div className="flex flex-col fluid-gap">
        <div className="flex items-center fluid-gap flex-wrap">
          {/* Quantity selector */}
          <div className="flex items-center bg-[#F3F5F9] rounded-full" style={{ height: 'var(--button-height)' }}>
            <button
            ...
            <span className="px-4 md:px-6 font-semibold text-responsive">{quantity}</span>
            <button
          ...
          <button
            ...
            className="responsive-button flex items-center justify-center gap-2 bg-[#006B51] text-white flex-1 rounded-full font-semibold hover:bg-[#005541] transition-colors tracking-widest"
          >
        ...
        <div className="flex flex-wrap fluid-gap mt-4">
          <button
            onClick={() => {
            ...
            className={`responsive-button flex items-center justify-center gap-2 border border-[#B1AEA9] rounded-full flex-1 ${
          ...
          <button
            className="responsive-button flex items-center justify-center gap-2 border border-[#B1AEA9] rounded-full flex-1 bg-white"
      ...
      <div className="mt-8 flex flex-col">
        ...
        <div className="flex flex-wrap fluid-gap">
          {['Beauty & Care', 'Hair Treatment', 'Tag Name', 'Tag Name', 'Tag Name'].map((tag, index) => (
            <span
              key={index}
              className="px-3 py-1 border border-[#B1AEA9] rounded-full text-responsive-xs text-[#595959] font-medium"
            >
              {tag}
            </span>
          ))}
      ...
      {/* Product details */}
      <div className="mt-8">
        <h3 className="font-semibold text-black mb-3 text-responsive">Product Details:</h3>
        <p className="text-[#595959] text-responsive-xs leading-relaxed">
