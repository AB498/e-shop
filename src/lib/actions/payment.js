...
export async function initializePaymentForAdmin(orderId) {
  try {
    // Get session to check if user is authenticated
    const session = await getServerSession(authOptions);

    // Log session for debugging
    console.log('Admin payment reinitialize - Session:',
      session ? {
        user: {
          id: session.user?.id,
    ...
    if (!items.length) {
      return { error: 'No items found in this order', status: 400 };
    }
    ...
    // Get user details
    console.log('Fetching user details for ID:', order.user_id);
    let user = {};
    ...
    if (data.status !== 'SUCCESS') {
      ...
      return { error: data.failedreason || 'Payment initialization failed', status: 400 };
    }
    ...
    console.error('Admin payment reinitialization error:', error);
